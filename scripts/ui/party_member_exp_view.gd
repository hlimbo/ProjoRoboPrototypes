...
func _process(delta: float) -> void:
	...
	# only stops when a level up condition occurs
	if remaining_exp <= 0:
		return

	var level_increase = 0
	var delta_exp = exp_earned_per_level * delta # exp earned per frame
	# implicitly stops increasing exp earned by capping it at an upperbound
	...
	# update UI
	level_label.text = "lvl %d" % level
	curr_exp_label.text = "%f" % curr_exp
