...
## Variable's name in code to read from (need to find and obtain value for comparison)
...
static func parse_from_json_recursive(json_data: Dictionary, curr_depth: int, max_depth: int) -> RuleJsonObject:
	...
	# recursive step
	if [AND, OR, NOT].has(json_data["operator"]):
	...
	if value is float:
		rule_object.data_type = FLOAT
		rule_object.float_value = value
	elif value is int:
		rule_object.data_type = INT
		rule_object.int_value = value
	elif value is bool:
		rule_object.data_type = BOOL
		rule_object.bool_value = value
	elif value is String:
		rule_object.data_type = STRING
